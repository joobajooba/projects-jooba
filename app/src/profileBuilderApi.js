...
export async function updateProfile(ownerWallet, updates) {
  ...
  const normalized = ownerWallet.toLowerCase();
  const { error } = await supabase.from('profile_page').update(updates).eq('owner_wallet', normalized);
  if (error) {
...
export async function incrementProfileView(profileId) {
  ...
  const { error } = await supabase.from('profile_page_views').insert({ profile_page_id: profileId });
