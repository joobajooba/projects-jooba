...
export default function EditProfilePanel() {
  ...
  const handleImageUploaderUpload = async (e) => {
    ...
    const rateLimitKey = `edit_panel_image_uploader_${address.toLowerCase()}`;
    if (!checkRateLimit(rateLimitKey, 10, 60000)) {
    ...
    const path = `${walletAddress}/image-uploader-${Date.now()}.${ext}`;
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('users')
        .update({ image_uploader_url: publicUrl })
        .eq('wallet_address', walletAddress);

      if (updateError) {
        console.error('Error saving Image Uploader image:', updateError);
      ...
      <div className="edit-profile-panel" role="dialog" aria-labelledby="edit-profile-title">
        ...
        <div className="edit-profile-panel-body">
          ...
          <div className="edit-profile-field edit-profile-uploader-field">
            ...
            <p className="edit-profile-field-hint">Choose an image from your device. It will appear in the Image Uploader panel (above Profile Description) on your profile page.</p>
            <input
              ref={imageUploaderRef}
              type="file"
              accept="image/jpeg,image/png,image/gif,image/webp"
              className="edit-profile-file-input"
              onChange={handleImageUploaderUpload}
