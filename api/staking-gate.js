...
const COOKIE_NAME = 'staking_gate';
const COOKIE_SECRET = process.env.STAKING_GATE_SECRET || 'j00ba-staking-gate-v1';
...
function expectedToken(wallet) {
  return hmacValue(`staking-unlocked:${wallet}`);
}
...
function makeChallenge() {
  ...
  return `${issuedAt}.${hmacValue(`staking-challenge:${issuedAt}`)}`;
}

function challengeValid(nonce) {
  const [issuedAt, signature] = String(nonce || '').split('.');
  if (!issuedAt || !signature) return false;
  if (!tokensMatch(signature, hmacValue(`staking-challenge:${issuedAt}`))) return false;
...
export default async function handler(request, response) {
  ...
  if (!ADDRESS_PATTERN.test(walletAddress) || !isAllowedWallet(walletAddress)) {
    return response.status(403).json({ unlocked: false, error: 'This wallet cannot open Staking.' });
  }
